...
In this first I run a simulation where the parameters for the two prey are the same. After this I was curious what happens if I set the predatory conversion rate and the predatory gain for prey 2 to a big higher value (see table ...). Biologically it would mean the predator catch that particular prey more easily and gain more from them, and also decrease the their population faster. I expect that the second prey population will decrease and converge to a value near zero, while the first prey population remains unchanged or a bit decreased compared to the unmodified system.
...
out.m <- model_system(LotVmod_twoprey, c(alpha1 = 0.1, beta1 = 0.02, gamma = 0.4, delta1 = 0.02, K1 = 100, tau1 = 0.3,alpha2 = 0.15, beta2 = 0.025, K2 = 100, tau2 = 0.3, delta2 = 0.04, sigma1 = 0.01, sigma2 = 0.01), 
                      c(x1 = 30,x2 = 30 ,y = 10), TRUE)
out.m2 <- model_system(LotVmod_twoprey, c(alpha1 = 0.1, beta1 = 0.02, gamma = 0.4, delta1 = 0.02, K1 = 100, tau1 = 0.3,alpha2 = 0.15, beta2 = 0.02, K2 = 100, tau2 = 0.3, delta2 = 0.02, sigma1 = 0.01, sigma2 = 0.01), 
...
params4 <- data.frame(parameter = c("a1", "a2", "b1", "b2", "K1","K2","tau1","tau2","l","delta1","delta2","X1_0","X2_0","Y0"), values =
                        c(0.1,0.15,0.02,0.025,100,100,0.3,
                          0.3,0.4, 0.02, 0.04, 30,10,10))
params5 <- data.frame(parameter = c("a1", "a2", "b1", "b2", "K1","K2","tau1","tau2","l","delta1","delta2","X1_0","X2_0","Y0"), values =
